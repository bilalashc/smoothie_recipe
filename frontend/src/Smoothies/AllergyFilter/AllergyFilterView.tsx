...
import { SmoothieCard } from "../ListSmoothies/components/SmoothieCard";
import { useNavigate } from 'react-router-dom';
import { getSmoothies } from "../../api/smoothieApi";
import './styles.css'
import { useDeleteSmoothie, useShareSmoothie } from '../utils/hooks';
import { DeleteModal } from '../ListSmoothies/components/DeleteModal';

export const AllergyFilterView = () => {
    const { smoothies, setSmoothies, loading, setLoading } = useSmoothiesContext();
    ...
    const filteredSmoothies = smoothies.filter(smoothie => {
        ...
        const smoothieIngredients = smoothie.ingredients.map(item => item.ingredient.name);
        return !selectedIngredients.some(allergen => smoothieIngredients.includes(allergen));
    });
                ...
                <div className="allergy-filter-section">
                    <p className="filter-instruction">
                        {selectedIngredients.length === 0 
                            ? "Select ingredients to filter out smoothies that cause you allergies:" 
                            : "Showing smoothies that don't contain your selected ingredients:"}
                    </p>
